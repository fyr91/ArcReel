...
**公司资产库（company asset catalog）**：
存放在 ArcReel 专用 Supabase 中、对公司账号可见的角色、场景与道具版本库；它统一承载监控服务导入的官方资产和用户从本地全局资产库发布的共享资产。
_Avoid_: 全局资产库、项目资产、为官方资产与共享资产各建一套库。

**官方资产（official company asset）**：
由资产源监控服务导入公司资产库的只读资产；创作者可以同步到本地使用，但不能从 ArcReel 客户端覆盖其公司版本。
_Avoid_: 用户共享资产、把“官方”理解为内置在安装包里的静态文件。

**共享资产（user-shared company asset）**：
用户把本地全局资产发布到公司资产库后形成的公司版本；首次发布与后续更新均保留稳定的公司资产身份并递增版本。
_Avoid_: 项目资产、上传即搬走本地文件。

**公司资产同步（company asset sync）**：
先用公司资产库某一类型的当前清单与本地全局资产库对账，再从清单水位之后增量追赶并更新本地检查点；因此公司资产在本地被删除后，下次同步会重新恢复。登录后自动同步全部类型，资产页手动同步只处理当前类型。同步得到的是本地快照，之后本地编辑不会自动回写，须显式发布新版本。
_Avoid_: 资产源监控、把同步理解为永久双向实时绑定。

**资产源监控（asset source monitor）**：
部署在 Supabase 同一服务器、独立于 Supabase 容器的后台服务；按数据源检查新增、修改与缺席，将变化写入公司资产库并留下运行记录、检查点和错误状态。
_Avoid_: ArcReel 本地同步按钮、Supabase 数据库触发器、把监控进程塞进数据库容器。
