MESSAGES = {
    ...
    "narrator_reference_video_only": "默认旁白角色仅适用于剧情或课程的参考生视频项目",
    "narrator_character_invalid": "旁白角色必须是角色名或 null",
    "narrator_character_not_found": "旁白角色「{name}」未在角色集中登记",
    ...
    "course_episode_delete_not_course": "只有课程项目支持单独删除分集",
    "course_episode_delete_not_found": "课程分集 {episode} 不存在",
    "course_episode_delete_invalid_project": "课程分集 {episode} 的数据异常，无法安全删除",
    "course_episode_delete_protected_resource": "课程分集 {episode} 异常引用了资源库文件，已拒绝删除以保护资源库",
    "course_episode_delete_shared_file": "课程分集 {episode} 与其他分集共用文件，无法单独删除",
    "course_episode_delete_confirmation_required": "删除分集前必须先查看影响范围并二次确认",
    "course_episode_delete_confirmation_expired": "删除确认已过期，请重新查看影响范围并确认",
    "course_episode_delete_confirmation_invalid": "删除确认无效或不属于当前分集，请重新确认",
    "course_episode_delete_confirmation_stale": "确认后分集内容已发生变化，尚未删除；请重新查看影响范围并确认",
    ...
    "reference_storyboard_sheet_task_submitted": "视频单元「{unit_id}」的故事板生成任务已提交",
    "reference_storyboard_sheet_confirmed": "已确认视频单元「{unit_id}」的故事板，并提交 {count} 个关键分镜任务",
    "reference_storyboard_sheet_required": "视频单元「{unit_id}」必须先生成故事板",
    "reference_keyframe_plan_required": "视频单元「{unit_id}」的正式脚本尚未提取关键分镜，请重新生成该 Video Unit 文稿",
    "reference_keyframes_required": "视频单元「{unit_id}」的正式文稿尚未提取 Keyframes，请重新生成该 Video Unit 文稿",
    "reference_keyframe_images_required": "视频单元「{unit_id}」仍缺少以下关键分镜首帧：{keyframe_ids}",
    "reference_storyboard_sheet_confirmation_required": "视频单元「{unit_id}」的故事板尚未确认",
    "reference_storyboard_sheet_input_changed": "视频单元「{unit_id}」在故事板生成期间发生变化，请重新生成",
    "reference_storyboard_sheet_changed": "视频单元「{unit_id}」的故事板已变化，请刷新后重新确认",
    "reference_storyboard_sheet_reference_limit": "视频单元「{unit_id}」确认故事板后共有 {count} 张参考图，超过模型上限 {max_count}；请拆分单元或减少引用",
    "reference_storyboard_sheet_keyframes_enqueue_failed": "故事板已确认，但视频单元「{unit_id}」有 {failed_count} 个关键分镜任务未能提交",
    ...
    "video_guided_continuation_unsupported": "{provider}/{model} 不支持 Guided 视频续接；请将该视频通道切换为 Croco MiniMax H3",
    "video_guided_continuation_mode_unsupported": "模型 {model} 不能同时使用 Guided 续接和硬首帧模式；请把当前分镜图作为参考图使用",
    "video_guided_continuation_end_frame_unsupported": "模型 {model} 不能同时使用 Guided 续接和固定尾帧；请先移除该单元的尾帧",
    "video_guided_source_unavailable": "GPU 前序任务 {source_job_id} 无法用于 Guided 续接（{reason}）；请先用 Croco MiniMax H3 重新生成前序单元",
    ...
    "video_hd_backend_unavailable": "MiniMax H3 高清处理后端暂时不可用，请稍后重试",
    "video_hd_failed": "高清处理失败：{unit_ids}；已保留原 480P 视频，请重试",
    ...
    "project_path_invalid": "只能定位当前项目内的相对路径",
    "project_path_not_found": "要定位的项目文件或文件夹不存在",
    "local_file_manager_unavailable": "无法打开本机文件管理器；请确认服务直接运行在本机桌面环境中",
    ...
    "config_import_disabled": "当前实例未启用配置文件导入",
    "config_bundle_read_failed": "无法读取配置文件",
    ...
    "config_bundle_file_credential_unsupported": "当前实例包含文件型渠道凭据，暂时无法导出为 .env.release",
    "config_bundle_provider_unknown": "配置文件包含当前版本不支持的渠道",
