MESSAGES = {
    ...
    "narrator_reference_video_only": "Nhân vật dẫn chuyện mặc định chỉ dùng cho dự án video tham chiếu dạng phim hoặc khóa học",
    "narrator_character_invalid": "Nhân vật dẫn chuyện phải là tên nhân vật hoặc null",
    "narrator_character_not_found": "Nhân vật dẫn chuyện '{name}' chưa được đăng ký trong bộ nhân vật",
    ...
    "course_episode_delete_not_course": "Chỉ dự án khóa học mới hỗ trợ xóa riêng từng tập",
    "course_episode_delete_not_found": "Không tồn tại tập khóa học {episode}",
    "course_episode_delete_invalid_project": "Dữ liệu của tập khóa học {episode} không hợp lệ nên không thể xóa an toàn",
    "course_episode_delete_protected_resource": "Tập khóa học {episode} tham chiếu bất thường đến tệp trong thư viện; thao tác xóa đã bị từ chối để bảo vệ thư viện tài nguyên",
    "course_episode_delete_shared_file": "Tập khóa học {episode} dùng chung tệp với tập khác nên không thể xóa riêng",
    "course_episode_delete_confirmation_required": "Hãy xem phạm vi ảnh hưởng và xác nhận lần hai trước khi xóa tập",
    "course_episode_delete_confirmation_expired": "Xác nhận xóa đã hết hạn; hãy xem lại phạm vi và xác nhận lại",
    "course_episode_delete_confirmation_invalid": "Xác nhận xóa không hợp lệ hoặc thuộc tập khác; hãy xác nhận lại",
    "course_episode_delete_confirmation_stale": "Nội dung tập đã thay đổi sau khi xác nhận nên chưa bị xóa; hãy xem lại phạm vi và xác nhận lại",
    ...
    "reference_storyboard_sheet_task_submitted": "Đã gửi tác vụ tạo bảng phân cảnh cho đơn vị video '{unit_id}'",
    "reference_storyboard_sheet_confirmed": "Đã xác nhận bảng phân cảnh của đơn vị video '{unit_id}' và gửi {count} tác vụ khung hình chính",
    "reference_storyboard_sheet_required": "Đơn vị video '{unit_id}' phải có bảng phân cảnh trước khi tiếp tục",
    "reference_keyframe_plan_required": "Bản thảo chính thức của Video Unit '{unit_id}' chưa có keyframe được trích xuất; hãy tạo lại bản thảo Video Unit",
    "reference_keyframes_required": "Bản thảo chính thức của Video Unit '{unit_id}' chưa có Keyframes được trích xuất; hãy tạo lại bản thảo Video Unit",
    "reference_keyframe_images_required": "Video Unit '{unit_id}' vẫn thiếu ảnh đã tạo cho các keyframe: {keyframe_ids}",
    "reference_storyboard_sheet_confirmation_required": "Bảng phân cảnh của đơn vị video '{unit_id}' chưa được xác nhận",
    "reference_storyboard_sheet_input_changed": "Đơn vị video '{unit_id}' đã thay đổi trong khi tạo bảng phân cảnh; hãy tạo lại",
    "reference_storyboard_sheet_changed": "Bảng phân cảnh của đơn vị video '{unit_id}' đã thay đổi; hãy tải lại và xác nhận lại",
    "reference_storyboard_sheet_reference_limit": "Xác nhận bảng phân cảnh của đơn vị video '{unit_id}' sẽ dùng {count} ảnh tham chiếu, vượt giới hạn {max_count}; hãy tách đơn vị hoặc giảm tham chiếu",
    "reference_storyboard_sheet_keyframes_enqueue_failed": "Bảng phân cảnh đã được xác nhận nhưng không thể gửi {failed_count} tác vụ khung hình chính của đơn vị video '{unit_id}'",
    ...
    "video_guided_continuation_unsupported": "{provider}/{model} không hỗ trợ nối tiếp video Guided; hãy chuyển luồng video này sang Croco MiniMax H3",
    "video_guided_continuation_mode_unsupported": "Mô hình {model} không thể kết hợp nối tiếp Guided với chế độ khung hình đầu cố định; hãy dùng storyboard hiện tại làm ảnh tham chiếu",
    "video_guided_continuation_end_frame_unsupported": "Mô hình {model} không thể kết hợp nối tiếp Guided với khung hình cuối cố định; hãy bỏ khung hình cuối trước khi tạo",
    "video_guided_source_unavailable": "Tác vụ GPU nguồn {source_job_id} không thể dùng cho nối tiếp Guided ({reason}); hãy tạo lại đơn vị nguồn bằng Croco MiniMax H3",
    ...
    "video_hd_backend_unavailable": "Dịch vụ HD MiniMax H3 tạm thời không khả dụng; hãy thử lại sau",
    "video_hd_failed": "Xử lý HD thất bại cho {unit_ids}; video 480P gốc vẫn được giữ lại. Hãy thử lại",
    ...
    "project_path_invalid": "Chỉ có thể định vị đường dẫn tương đối bên trong dự án hiện tại",
    "project_path_not_found": "Tệp hoặc thư mục dự án cần định vị không tồn tại",
    "local_file_manager_unavailable": "Không thể mở trình quản lý tệp cục bộ; hãy đảm bảo dịch vụ đang chạy trực tiếp trong môi trường máy tính để bàn cục bộ",
    ...
    "config_import_disabled": "Phiên bản này chưa bật nhập tệp cấu hình",
    "config_bundle_read_failed": "Không thể đọc tệp cấu hình",
    ...
    "config_bundle_file_credential_unsupported": "Phiên bản này có thông tin xác thực kênh dạng tệp chưa thể xuất sang .env.release",
    "config_bundle_provider_unknown": "Cấu hình chứa kênh không được phiên bản này hỗ trợ",
