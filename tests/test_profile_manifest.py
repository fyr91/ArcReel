...
def test_enumerate_profile_files_includes_claude_variants(tmp_path: Path) -> None:
    profile = tmp_path / "profile"
    profile.mkdir()
    (profile / "CLAUDE.course.md").write_text("n")
    (profile / "CLAUDE.drama.md").write_text("d")

    files = enumerate_profile_files(profile)
    assert files == {"CLAUDE.course.md", "CLAUDE.drama.md"}
...
def test_enumerate_profile_files_ignores_unrelated_top_files(tmp_path: Path) -> None:
    ...
    (profile / "foo.course.md").write_text("noise variant")  # 非 CLAUDE 家族变体
...
def test_valid_content_modes_constant() -> None:
    ...
    assert VALID_CONTENT_MODES == frozenset({"course", "drama", "ad"})
...
def _make_profile(tmp_path: Path) -> Path:
    """构造典型 profile：通用文件 + course/drama/ad 变体配对。"""
    profile = tmp_path / "profile"
    ...
    # CLAUDE.md 变体配对
    (profile / "CLAUDE.course.md").write_text("course top")
    (profile / "CLAUDE.drama.md").write_text("drama top")
    (profile / "CLAUDE.ad.md").write_text("ad top")
    # SKILL.md 变体配对
    (profile / ".claude" / "skills" / "video-workflow" / "SKILL.course.md").write_text("nar skill")
...
def test_resolve_for_course_picks_course_variants(tmp_path: Path) -> None:
    from lib.profile_manifest import resolve_profile_files_for_mode

    profile = _make_profile(tmp_path)
    mapping = resolve_profile_files_for_mode(profile, "course")

    assert mapping == {
        "CLAUDE.md": "CLAUDE.course.md",
        ".claude/agents/generate-assets.md": ".claude/agents/generate-assets.md",
        ".claude/skills/video-workflow/SKILL.md": ".claude/skills/video-workflow/SKILL.course.md",
    }
...
def test_resolve_common_plus_variant_collision_raises(tmp_path: Path) -> None:
    ...
    (profile / "CLAUDE.course.md").write_text("variant")
...
def test_resolve_double_dot_filename_not_treated_as_variant(tmp_path: Path) -> None:
    """`foo.course.bar.md` 不认作变体（只识别最后一段 stem）。"""
    from lib.profile_manifest import resolve_profile_files_for_mode

    profile = tmp_path / "profile"
    (profile / ".claude").mkdir(parents=True)
    (profile / ".claude" / "weird.course.bar.md").write_text("not a variant")

    mapping = resolve_profile_files_for_mode(profile, "course")
    assert mapping == {".claude/weird.course.bar.md": ".claude/weird.course.bar.md"}
...
def test_manifest_serialize_includes_set_content_mode() -> None:
    ...
    data = json.loads(m.normalized_bytes().decode("utf-8"))
    assert data["content_mode"] == "course"
...
def test_sync_course_project_writes_course_variant(tmp_path: Path) -> None:
    from lib.profile_manifest import sync_profile_to_project

    profile = _make_profile(tmp_path)
    project = _fresh_project(tmp_path / "proj_root")

    sync_profile_to_project(profile, project, content_mode="course")

    assert (project / "CLAUDE.md").read_text() == "course top"
    assert (project / ".claude" / "skills" / "video-workflow" / "SKILL.md").read_text() == "nar skill"
    assert not (project / "CLAUDE.course.md").exists()
...
def test_sync_writes_manifest_content_mode(tmp_path: Path) -> None:
    ...
    assert manifest_data["content_mode"] == "course"


def test_sync_mode_mismatch_triggers_reset(tmp_path: Path) -> None:
    """已有 manifest 标记 course，下次 sync 传 drama → reset 路径覆盖 dest。"""
    from lib.profile_manifest import sync_profile_to_project
...
def test_sync_legacy_manifest_migrates_without_reset(tmp_path: Path) -> None:
    ...
    assert after["content_mode"] == "course"
    # 内容不变
    assert (project / "CLAUDE.md").read_text() == "course top"
...
def test_force_resync_picks_correct_variant(tmp_path: Path) -> None:
    ...
    # force_resync 应当用 course 变体覆盖
    force_resync_profile(profile, project, content_mode="course", paths=["CLAUDE.md"])
    assert (project / "CLAUDE.md").read_text() == "course top"
...
@pytest.mark.parametrize("mode", ["course", "drama", "ad"])
def test_variants_to_common_upgrade_preserves_modified_legacy_files_and_reports_them(tmp_path: Path, mode: str) -> None:
    ...
    references.mkdir()
    for variant in ("course", "drama", "ad"):
...
def test_full_force_reset_discards_all_customized_files_and_restores_builtin(tmp_path: Path) -> None:
    ...
    assert get_profile_status(profile, project, content_mode="course") == {
        "customized": True,
        "customized_files": [".claude/skills/private/SKILL.md", "CLAUDE.md"],
    }

    force_resync_profile(profile, project, content_mode="course")

    assert (project / "CLAUDE.md").read_text(encoding="utf-8") == "course top"
...
def test_profile_status_rejects_symlinked_profile_tree(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="profile tree is a symlink"):
        get_profile_status(profile, project, content_mode="course")
...
def test_create_project_default_is_drama(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """未传 content_mode 时按当前产品默认使用 drama。"""
    pm, _ = _setup_pm_with_profile(tmp_path, monkeypatch)
...
def test_sync_agent_profile_reads_content_mode_from_project_json(
    ...
    pm, _ = _setup_pm_with_profile(tmp_path, monkeypatch)
    project_dir = pm.create_project("demo", content_mode="course")
...
def test_sync_agent_profile_missing_mode_raises(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    pm, _ = _setup_pm_with_profile(tmp_path, monkeypatch)
    project_dir = pm.create_project("demo", content_mode="course")
    # 模拟老项目：project.json 没有 content_mode 字段
    pj_path = project_dir / "project.json"
    pj_path.write_text(json.dumps({"title": "demo"}))
    with pytest.raises(ValueError, match="no content_mode"):
        pm.sync_agent_profile(project_dir)
...
def test_sync_agent_profile_corrupt_json_raises(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """project.json 损坏 → raise，避免 drama 项目被静默回退到 course 触发
...
def test_sync_all_agent_profiles_isolates_corrupt_project(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    损坏项目的 manifest 不被错误切回 course（不触发破坏性 reset）。"""
    pm, _ = _setup_pm_with_profile(tmp_path, monkeypatch)
    pm.create_project("good", content_mode="course")
    ...
    assert (pm.projects_root / "good" / "CLAUDE.md").read_text() == "course top"
    # 损坏项目的 CLAUDE.md 保持上次 sync 的 drama 内容，未被错切回 course
...
def test_sync_all_agent_profiles_per_project_mode(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    pm, _ = _setup_pm_with_profile(tmp_path, monkeypatch)
    pm.create_project("a", content_mode="course")
    pm.create_project("b", content_mode="drama")
    # 改两个项目的内容（模拟 server 启动前 profile 已升级）
    stats = pm.sync_all_agent_profiles()
    assert stats.get("aborted") is not True
    assert (pm.projects_root / "a" / "CLAUDE.md").read_text() == "course top"
